...
void test_runge_kutta() {
	...
	for(unsigned int i{0}; i < npassi; i++) {
		x = rk.Passo(t, x, h, oscarm);
		t = t + h;
	}

	assert(is_close(x[0], 0.7738501114078689));
	assert(is_close(x[1], 0.6333611095194112));

	std::cerr << "test runge-kutta ok" << std::endl;
}

void test_interpolazione() {
	std::vector<double> a {-0.4,-0.7};
	std::vector<double> b {0.5,0.8};

	assert(is_close(interpolazione(a,b,0.3), 0.2));
	std::cerr << "test interpolazione ok" << std::endl;
}